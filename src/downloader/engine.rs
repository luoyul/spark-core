...
impl DownloadEngine {
    ...
    pub fn new(config: DownloadConfig) -> Result<Self> {
        let client =
            HttpClient::new().map_err(|e| anyhow!("Failed to create HTTP client: {}", e))?;
    ...
    pub async fn download(&self, url: &str, output: &Path) -> Result<()> {
        ...
        info!(
            "Total chunks: {}, IO threads: {}",
            chunk_count, self.config.io_thread_count
        );
        ...
        loop {
            ...
            match task {
                Some(IoTask {
                    ...
                    debug!(
                        "IO worker writing chunk {} at offset {}",
                        chunk_index, offset
                    );
        ...
        loop {
            ...
            match Self::download_chunk(client, url, range, chunk_index, buffer_size, io_tx).await {
                ...
                Err(e) => {
                    ...
                    let delay =
                        tokio::time::Duration::from_millis(1000u64 * 2u64.pow(attempts as u32));
