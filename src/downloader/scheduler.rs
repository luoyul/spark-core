    ...
    async fn worker_loop(
        ...
        while running.load(Ordering::SeqCst) {
            ...
            if let Some(task) = task {
                ...
                // 执行下载任务 — 调用方通过注入 download_fn 实现
                // 当前版本：任务处理逻辑由外层 DownloadEngine 管理，
                // Scheduler 负责调度和分发
...
mod tests {
    ...
    #[test]
    fn test_byte_range() {
        let range = HttpRange::new_unchecked(0, 1024);
        assert_eq!(range.size(), 1024);

        let range = HttpRange::new_unchecked(100, 200);
